...
func (mw InstrumentingMiddleware) GenKey(ctx context.Context, bit int) (output string, err error) {
	defer func(begin time.Time) {
		lvs := []string{"method", "genkey", "error", "false"}
		mw.RequestCount.With(lvs...).Add(1)
		mw.RequestLatency.With(lvs...).Observe(time.Since(begin).Seconds())
	}(time.Now())

	output, err = mw.Next.GenKey(ctx, bit)
	return
}